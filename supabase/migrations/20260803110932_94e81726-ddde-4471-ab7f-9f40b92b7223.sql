-- ============================================================ status ladder
CREATE OR REPLACE FUNCTION public.wf_status_ladder()
RETURNS TABLE(status text, ord int)
...
AS $$
  SELECT * FROM (VALUES
    ...
    ('Tracing', 2),
    ('Located', 3),
    ('Arrived at Airport', 4),
    ('Waiting Customs Clearance', 5),
    ('Ready for Delivery', 6),
    ('Out for Delivery', 7),
    ('Delivered', 8),
    ('Returned to Airport', 9),
    ('Ready for Airport Pickup', 10),
    ('Passenger Picked Up', 11)
  ) AS t(status, ord)
$$;

REVOKE EXECUTE ON FUNCTION public.wf_status_ladder() FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.wf_status_ladder() TO authenticated, service_role;

-- Current status + journey type for every case. Single source of truth for
-- KPI buckets, distributions and pipelines across Dashboard and Reports.
...
AS $$
  SELECT c.id,
         ...
         c.created_at,
         CASE
           WHEN c.lf_status::text = 'Passenger Picked_Up' THEN c.resolved_at
           ELSE coalesce(d.delivered_at, c.resolved_at)
         END AS completed_at
...
BEGIN
  ...
  CREATE TEMP TABLE IF NOT EXISTS _dash_cases ON COMMIT DROP AS SELECT 1 WHERE false;

  -- --------------------------------------------------- current-status KPIs
  SELECT count(*) INTO k_total FROM public.wf_case_status();

  SELECT count(*) FILTER (WHERE s.status = 'Open'),
         count(*) FILTER (WHERE s.status = 'Located'),
    ...
    INTO k_open, k_located, k_arrived, k_customs, k_ready, k_out,
         k_returned, k_ready_pickup, k_picked, k_delivered
    FROM public.wf_case_status() s;

  -- "Open Cases" = every case that has not completed its journey.
  SELECT count(*) INTO k_open FROM public.wf_case_status() s
   WHERE s.status NOT IN ('Delivered','Passenger Picked Up');
  ...
  SELECT jsonb_build_object(
    ...
    -- full lifecycle, zero-filled, in operational order
    'byStatus', coalesce((
    ...
    -- unified operational pipeline (Lost & Found + Delivery + Airport Pickup)
    'funnel', coalesce((
      SELECT jsonb_agg(jsonb_build_object('status', l.status, 'count', x.n) ORDER BY l.ord)
      FROM public.wf_status_ladder() l
      CROSS JOIN LATERAL (
...
-- ============================================ lifecycle block for Reports
CREATE OR REPLACE FUNCTION public.report_lifecycle(
  p_from timestamptz, p_to timestamptz, p_grain text DEFAULT 'day', p_journey text DEFAULT 'all')
RETURNS jsonb
LANGUAGE plpgsql STABLE SECURITY DEFINER
SET search_path = public
AS $function$
DECLARE
  g text := CASE lower(coalesce(p_grain,'day')) WHEN 'week' THEN 'week' WHEN 'month' THEN 'month' ELSE 'day' END;
  j text := CASE coalesce(p_journey,'all')
              WHEN 'Home Delivery' THEN 'Home Delivery'
              WHEN 'Airport Pickup' THEN 'Airport Pickup'
              ELSE 'all' END;
  v jsonb;
BEGIN
  IF auth.uid() IS NULL THEN
  ...
  SELECT jsonb_build_object(
    ...
    'totals', jsonb_build_object(
      'cases', (SELECT count(*)::int FROM public.wf_case_status() s WHERE j = 'all' OR s.journey = j),
      'homeDelivery', (SELECT count(*)::int FROM public.wf_case_status() s WHERE s.journey = 'Home Delivery'),
      'airportPickup', (SELECT count(*)::int FROM public.wf_case_status() s WHERE s.journey = 'Airport Pickup'),
      'delivered', (SELECT count(*)::int FROM public.wf_case_status() s
                     WHERE s.status = 'Delivered' AND (j = 'all' OR s.journey = j)),
      'pickedUp', (SELECT count(*)::int FROM public.wf_case_status() s
                    WHERE s.status = 'Passenger Picked Up' AND (j = 'all' OR s.journey = j)),
      'completed', (SELECT count(*)::int FROM public.wf_case_status() s
                     WHERE s.status IN ('Delivered','Passenger Picked Up') AND (j = 'all' OR s.journey = j)),
      'returned', (SELECT count(*)::int FROM public.wf_case_status() s
                    WHERE s.status = 'Returned to Airport' AND (j = 'all' OR s.journey = j)),
      'deliverySuccessPct', coalesce((
        SELECT CASE WHEN count(*) FILTER (WHERE s.status IN ('Delivered','Returned to Airport')) = 0 THEN 0
                    ELSE round(100.0 * count(*) FILTER (WHERE s.status = 'Delivered')
                         / count(*) FILTER (WHERE s.status IN ('Delivered','Returned to Airport')), 1) END
          FROM public.wf_case_status() s WHERE s.journey = 'Home Delivery'), 0),
      'pickupSuccessPct', coalesce((
        SELECT CASE WHEN count(*) = 0 THEN 0
                    ELSE round(100.0 * count(*) FILTER (WHERE s.status = 'Passenger Picked Up') / count(*), 1) END
          FROM public.wf_case_status() s WHERE s.journey = 'Airport Pickup'), 0)),
    'pipeline', coalesce((
      SELECT jsonb_agg(jsonb_build_object('status', l.status, 'count', x.n) ORDER BY l.ord)
      FROM public.wf_status_ladder() l
      CROSS JOIN LATERAL (
        SELECT count(*)::int AS n FROM public.wf_case_status() s
         WHERE s.status = l.status AND (j = 'all' OR s.journey = j)) x), '[]'::jsonb),
    'byJourney', coalesce((
      SELECT jsonb_agg(jsonb_build_object('journey', y.journey, 'count', y.n) ORDER BY y.journey)
      FROM (SELECT s.journey, count(*)::int AS n FROM public.wf_case_status() s GROUP BY 1) y), '[]'::jsonb),
    'trends', coalesce((
      SELECT jsonb_agg(jsonb_build_object(
               'bucket', to_char(b.bucket, 'YYYY-MM-DD'),
               'delivered', b.delivered, 'pickedUp', b.picked_up,
               'completed', b.delivered + b.picked_up) ORDER BY b.bucket)
      FROM (
        SELECT d.bucket,
          (SELECT count(*) FROM public.workflow_events w
             JOIN public.baggage_cases c ON c.id = w.case_id
            WHERE w.to_status = 'DELIVERED' AND date_trunc(g, w.occurred_at) = d.bucket
              AND (j = 'all' OR c.delivery_method::text = j)) AS delivered,
          (SELECT count(*) FROM public.workflow_events w
             JOIN public.baggage_cases c ON c.id = w.case_id
            WHERE w.to_status = 'PASSENGER_PICKED_UP' AND date_trunc(g, w.occurred_at) = d.bucket
              AND (j = 'all' OR c.delivery_method::text = j)) AS picked_up
        FROM (SELECT generate_series(date_trunc(g, p_from), date_trunc(g, p_to - interval '1 microsecond'),
                                     ('1 ' || g)::interval) AS bucket) d
      ) b), '[]'::jsonb)
  ) INTO v;
...
REVOKE EXECUTE ON FUNCTION public.report_lifecycle(timestamptz, timestamptz, text, text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.report_lifecycle(timestamptz, timestamptz, text, text) TO authenticated, service_role;