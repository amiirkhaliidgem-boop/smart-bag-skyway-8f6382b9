...
import { loadRegions, resolveRegion, regionNames } from "./regions";
...
const lostFoundFields: FieldDef[] = [
  ...
  { key: "mobile1", label: "Mobile 1", type: "phone", required: true, example: "+201002345512" },
  { key: "mobile2", label: "Mobile 2", type: "phone", example: "+447911123456" },
  { key: "email", label: "Email", type: "email", example: "passenger@example.com" },
  { key: "pnr", label: "PNR", type: "string", example: "X7K2QA" },
  { key: "airline", label: "Airline (ICAO)", type: "airlineCode", required: true, example: "ABY" },
  ...
  { key: "destination", label: "Destination", type: "airportCode", required: true, example: "CAI" },
  {
    ...
    multiValueSeparator: ",",
    existingValues: (rec) => {
      const bag = (rec as { baggage?: { bagTags?: unknown[] } }).baggage;
      const tags = Array.isArray(bag?.bagTags) ? bag!.bagTags!.map(String) : [];
      const legacy = rec["bagTagNumber"];
      if (legacy) tags.push(String(legacy));
      return tags.map((t) => t.trim()).filter(Boolean);
    },
    example: "MS548921,MS548922",
  },
...
export const lostFoundSchema: DatasetSchema = {
  ...
  apply: async (rows: Record<string, unknown>[], ctx) => {
    ...
    for (const raw of rows) {
      ...
      // Multiple bag tags may arrive in one cell — one case, many bags.
      const bagTags = String(raw.bagTag ?? "")
        .split(",")
        .map((t) => t.trim())
        .filter(Boolean);
      // Region must match a configured SLA Region. Unknown values never
      // reject the row: the case is created and flagged for completion.
      const region = resolveRegion(raw.region);
      if (!region && !isBlank(raw.region) && !missingFields.includes("Region")) {
        missingFields.push("Region");
      }
      const incomplete = missingFields.length > 0;
      const bagTag = bagTags[0] ?? "";
      const declared = Number(raw.numberOfBags);
      const numBags = declared > 0 ? Math.max(declared, bagTags.length) : Math.max(bagTags.length, 1);
      ...
      try {
        ...
        baggage: {
          ...
          bagTags: bagTags.length ? bagTags : [bagTag],
        },
        delivery: {
          method,
